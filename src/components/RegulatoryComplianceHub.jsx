...
export default function RegulatoryComplianceHub({
  ...
  console.log(country);
  const projectName = state?.projectName ?? projectNameProp;
  // Sanju - 01/04/2026 — wrap gotoPhase to persist P4 data before navigating away (bottom-to-top fix)
  const gotoPhaseRaw = usePhaseNavigation(projectId, projectName, country);
  // Sanju - 01/04/2026 — async wrapper saves P4 segments to DB before sidebar navigation
  const gotoPhase = async (phaseId) => {
    if (projectId && hasHydrated) {
      try {
        await updateProjectMeta(projectId, { segmentsP4: buildP4Segments() });
      } catch (e) {
        console.warn('Sanju - 01/04/2026: Failed to persist P4 before navigation:', e);
      }
    }
    gotoPhaseRaw(phaseId);
  };
  ...
  /** Normalize incoming segments from router state */
  // Sanju - 01/04/2026 — changed priority: persistedP4 FIRST so returning to this page reads its own saved data
  const segments = useMemo(() => {
    const rawCandidate =
      // Sanju - 01/04/2026 — persistedP4 takes highest priority for bottom-to-top state retention
      (Array.isArray(persistedSegmentsP4) && persistedSegmentsP4.length > 0)
        ? persistedSegmentsP4
        : (Array.isArray(state?.segments) && state.segments.length > 0)
          ? state.segments
    ...
    return (rawCandidate || [])
      .map((seg, i) => {
        ...
        return {
          ...
          lang: seg.lang ?? "EN",
          complianceScore: typeof seg.complianceScore === "number" ? seg.complianceScore : null,
          // Sanju - 01/04/2026 — carry forward P4-specific override fields from persisted DB data
          compliantText: seg.compliantText ?? "",
          mlr: seg.mlr ?? null,
          mlrDefer: seg.mlrDefer ?? null,
          acceptedRisk: seg.acceptedRisk ?? null,
          blocked: seg.blocked ?? null,
        };
      })
      .filter((s) => s.source.trim().length > 0)
      .sort((a, b) => a.index - b.index);
  ...
  useEffect(() => {
    ...
    segments.forEach((s) => {
      if (typeof s.complianceScore === "number") initialScores[s.id] = s.complianceScore;
      if (s.compliant?.trim()) initialCompliant[s.id] = s.compliant;
      // Sanju - 01/04/2026 — also hydrate compliantText from persisted P4 segment data
      if (s.status || s.compliantText || s.mlr || s.mlrDefer || s.acceptedRisk || s.blocked) {
        initialOverrides[s.id] = {
          ...(s.status ? { status: s.status } : {}),
          ...(s.compliantText ? { compliantText: s.compliantText } : {}),
...
const isAcceptGroup = decisionGroup === "accept";
  ...
  /** Sidebar: retain context when moving across phases 31_03_sanju */
  // Sanju - 01/04/2026 — handlePhaseClick now delegates to gotoPhase which persists P4 first
  const handlePhaseClick = (phaseName) => {
    if (phaseName === "Global Context Capture") {
      // Sanju - 01/04/2026 — use gotoPhase wrapper instead of direct navigate to persist P4 data
      gotoPhase('P1');
    }
    if (phaseName === "Smart TM Translation") {
      // Sanju - 01/04/2026 — use gotoPhase wrapper instead of direct navigate to persist P4 data
  ...
  //Kathir-25/3
    ...
    if (!projectId) return;
    try {
      // Sanju - 01/04/2026 — persist P4 segments before marking phase complete
      await updateProjectMeta(projectId, { segmentsP4: buildP4Segments() });
    ...
    {SIDEBAR_PHASES.map((p) => {
      ...
      <div className="tm-main">
        ...
        {mainTab === "review" && (
          <div>
            {/*sanju_01_04 */}
            <section className="tm-tabs-right" style={{ display: 'flex', alignItems: 'center', gap: '5px' }}> 
  <div className="tm-status-left">
    <h2 className="tm-section-title">
      <Shield size={20} className="h-5 w-5" />
      Regulatory Compliance Workspace
